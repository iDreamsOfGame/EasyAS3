...
package org.easyas3.utils
{
	...
	public final class StringUtil
	{
		 ...
		 */
		public static function getCamelCaseString(source:String, dealWithAllChars:Boolean = true, delimiter:* = null):String
		{
		...
		public static function getTimeStr(time:Number = 0, withHour:Boolean = false, sep:String = ":"):String
		{
			...
			time /= MSEL_STEP;
			hour = MathUtil.floor(time / Math.pow(TIME_STEP, 2));
			min = MathUtil.floor(time / TIME_STEP) % TIME_STEP;
			sec = MathUtil.floor(time % TIME_STEP);
		 ...
		 * @return	String — 得到被格式化后的字符串
		 */
		public static function getUpperCamelCase(source:String, dealWithAllChars:Boolean = true, delimiter:* = null):String
		{
			var sourceArray:Array = source.split(delimiter);
			var copuString:String;
			var len:int = sourceArray.length;
			...
			{
				copuString = sourceArray[0];
				sourceArray[0] = copuString.substr(0, 1).toUpperCase() + (dealWithAllChars ? copuString.substring(1).toLowerCase() : copuString.substring(1));
				...
				if(len > 1)
				{
					...
					{
						var item:String = sourceArray[i];
						sourceArray[i] = item.substr(0, 1).toUpperCase() + (dealWithAllChars ? item.substring(1).toLowerCase() : copuString.substring(1));
					}
				}
			}
