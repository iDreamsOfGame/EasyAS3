		...
		protected function gatherParameterValues(target : Object, injector : Injector) : Array
		{
			...
			for(var i : int = 0; i < length; i++)
			{
				...
				if(injection == null)
				{
					...
					throw(new InjectorError('Injector is missing a rule to handle injection into target ' + target + '. Target dependency: ' + getQualifiedClassName(config.request) + ', method: ' + methodName + ', parameter: ' + (i + 1)));
				}
