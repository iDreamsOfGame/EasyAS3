...
package com.greensock
{
	...
	public class TweenMax extends TweenLite implements IEventDispatcher
	{
		...
		{
			if(n == 0)
			{ //can't allow zero because it'll throw the math off
				n = 0.0001;
			}
	   ...
	   * which includes repeats and repeatDelays (if there are any). For example, if a TweenMax instance
		...
		override public function renderTime(time:Number, suppressEvents:Boolean=false, force:Boolean=false) : void
		{
			...
			else if(time <= 0)
			{
				...
				{ //if we render the very beginning (time == 0) of a TweenMax.fromTo(), we must force the render (normal tweens wouldn't need to render at a time of 0 when the prevTime was also 0). This is also mandatory to make sure overwriting kicks in immediately.
					force = true;
				}
