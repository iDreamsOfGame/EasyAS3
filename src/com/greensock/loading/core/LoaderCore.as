...
package com.greensock.loading.core
{
	...
	public class LoaderCore extends EventDispatcher
	{
		...
		public function load(flushContent:Boolean=false) : void
		{
			...
			{ //use this.status instead of _status so that LoaderMax instances have a chance to do their magic in the getter and make sure their status is calibrated properly in case any of its children changed status after the LoaderMax completed (maybe they were manually loaded or failed, etc.).
			...
			if(_status == LoaderStatus.READY)
			{
				...
				_load();
				if(this.progress < 1)
				{ //in some cases, an OPEN event should be dispatched, like if load() is called on an empty LoaderMax, it will just dispatch a PROGRESS and COMPLETE event right away. It wouldn't make sense to dispatch an OPEN event right after that.
